...
class MLPEncoder(nn.Module):
    def __init__(self, in_dim, emb_dim=256, proj_dim=256):
        super().__init__()
        self.encoder = nn.Sequential(
            nn.Linear(in_dim, 512), nn.BatchNorm1d(512), nn.GELU(), nn.Dropout(0.2),
            nn.Linear(512,    512), nn.BatchNorm1d(512), nn.GELU(), nn.Dropout(0.2),
            nn.Linear(512,    256), nn.BatchNorm1d(256), nn.GELU(),
            nn.Linear(256, emb_dim),
        )
        self.projector = nn.Sequential(
            nn.Linear(emb_dim, 512), nn.BatchNorm1d(512), nn.ReLU(),

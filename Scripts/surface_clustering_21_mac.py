...
CONFIG = {
    ...
    "vcn_epochs"         : 150,
    "vcn_batch_size"     : 64,   # larger batches improve MPS throughput
    "vcn_lr"             : 1e-3,
    "vcn_patience"       : 50,
...
class FAAG(nn.Module):
    ...
    def forward(self, x):
        ...
        mag    = torch.fft.rfft(x.contiguous(), dim=-1).abs()     # (B, C, freq_bins)
...
def multi_rec_loss(x_orig, recon, enc_inter, dec_inter):
    ...
    # L1: main reconstruction — F.mse_loss already averages over all elements
...
def main():
    ...
    if hasattr(torch, "compile"):
        try:
            # "aot_eager": AOT tracing without Metal shader codegen — avoids
            # inductor threadgroup-memory limit and complex-op errors on MPS
            model = torch.compile(model, backend="aot_eager")
            print("  torch.compile enabled (aot_eager)")
        except Exception as e:

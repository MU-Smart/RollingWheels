"""
Road Surface Classification — VibClustNet Clustering Pipeline (Result Only)

Model      : VibClustNet from script 24 (rec_head temporal reconstruction)
Checkpoint : vibclustnet_best_24.pth  (no training — result-only mode)

Steps
-----
1. Load windowed pkl  →  Z-normalise  →  stratified 80/20 split (labeled only)
...
8. VibClustNet diagnostic plots (temporal attention, CAIM, reconstruction)
9. Repeat clustering / evaluation / visualisation on unlabeled data (unsupervised)
"""
...
def setup_logging(log_dir: Path = Path(".")) -> logging.Logger:
    ...
    log_path = log_dir / f"run_{RUN_TS}_24_cosine_results.log"
...
CONFIG = {
    ...
    "output_csv"             : Path("24_cosine_results_unlabeled_predictions.csv"),
    "unlabeled_id"           : 0,
...
# ══════════════════════════════════════════════════════════════════════════════
# VibClustNet — script-24 architecture (rec_head temporal reconstruction)
# ══════════════════════════════════════════════════════════════════════════════
...
class VibClustNet(nn.Module):
    ...
    Reconstruction is performed from the full-resolution temporal feature map
    (after3_temporal) via rec_head — avoids the predict-zero collapse of
    embedding-based decoders on Z-normalised data.
    """
    def __init__(self, T: int, emb_dim: int = 256, n_classes: int = 0):
        ...
        self.faag   = FAAG(3 * CH, T)               # 3 × 96 = 288 channels
        ...
        # Reconstruction head applied to after3 temporal features (B, CH, T)
    ...
    def _encode(self, x):
        """x: (B, 3, T) → emb (B, emb_dim) + enc_inter dict."""
        axes = [x[:, i:i+1, :] for i in range(3)]              # 3 × (B, 1, T)
        axes = [self.mstcb2(self.mstcb1(a)) for a in axes]     # 3 × (B, 96, T)
        attended, caim_w = self.caim(axes)                      # 3 × (B, 96, T), (B, 3, 3)
        cat              = torch.cat(attended, dim=1)           # (B, 288, T)
        fout, t_attn, f_attn = self.faag(cat)                   # (B, 288, T)
        after3           = self.mstcb3(fout)                    # (B, 96, T)
        pooled = after3.mean(dim=-1)                            # (B, 96)
    ...
    def _decode(self, after3):
        """after3: (B, CH, T) temporal features before global pool → recon (B, 3, T)."""
    ...
    def embed(self, x):
        """Inference-time normalised embedding (no grad needed from caller)."""
        if x.shape[1] != 3:
            x = x.permute(0, 2, 1)
...
# ── Log-cosh reconstruction loss (reference only) ─────────────────────────────
...
def plot_vibclustnet_diagnostics(model, te_X, te_pred_kmeans, device, figures_dir: Path):
    ...
    Three diagnostic plots:
      ...
      2. CAIM cross-axis attention heatmap per cluster (3×3)
      3. Original vs reconstructed signal for 3 random test samples
    """
    model.eval()
    ...
    all_t_attn = np.concatenate(all_t_attn, axis=0)   # (N, 288, T)
...
def main():
    ...
    # ── [3] Load VibClustNet from checkpoint ───────────────────────────────────
    logger.info("\n[3] Load VibClustNet (script-24 model) from checkpoint")
    ...
    # ── [8] VibClustNet diagnostics (labeled test) ────────────────────────────
    logger.info("\n[8] VibClustNet diagnostic plots (labeled test)")
    ...
    if xu_pred is not None and xu_pca is not None:
        ...
        fname_ts_u = figures_dir / f"19_tsne_unlabeled_clusters_{RUN_TS}.png"
        _plot_proj_grid_unsupervised(ts_u, "t-SNE", xu_pred, fname_ts_u)
        plot_unlabeled_individual(ts_u, "t-SNE", "tsne", xu_pred, figures_dir)

        fname_um_u = figures_dir / f"19_umap_unlabeled_clusters_{RUN_TS}.png"
        ...
        fname_fk_ts = figures_dir / f"21_fixed_k_tsne_unlabeled_{RUN_TS}.png"
        rows_fk = _fixed_k_grid(xu_pca, ts_u, "t-SNE", fname_fk_ts, (3, 5, 7, 11))
        _print_quality(rows_fk)

        fname_fk_um = figures_dir / f"21_fixed_k_umap_unlabeled_{RUN_TS}.png"
        rows_fk_um = _fixed_k_grid(xu_pca, um_u, "UMAP", fname_fk_um, (3, 5, 7, 11))
        _print_quality(rows_fk_um)

        logger.info("\n[9b] VibClustNet diagnostic plots (unlabeled)")
        plot_vibclustnet_diagnostics(model, X_u, xu_pred["kmeans"], device, figures_dir)
    else:

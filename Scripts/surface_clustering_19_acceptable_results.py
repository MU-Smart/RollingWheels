...
# boundaries between 6 physically meaningful categories instead of 12
# fine-grained ones.
# Surface merging rules (per domain knowledge — similar vibration patterns):
#   (1, 11)   → class 0  : Paving Blocks Smooth + Rectangular Tiles
#   (2)       → class 1  : Concrete Sidewalk       (unique)
#   (2, 5)    → class 1  : Concrete Sidewalk + Asphalt
#   (3, 4)    → class 2  : Brick — Smooth + Rough
#   (6, 7, 8) → class 3  : Indoor — Carpet/Lino/Tile
#   (9, 10)   → class 4  : Curb Up + Down
#   (12)      → class 5  : Paving Blocks Rough        (unique)
MERGE_MAP = {
    1:  0,   # Paving Blocks (Smooth) (Red)
   11:  0,   # Rectangular Paving Tiles
    2:  1,   # Concrete Sidewalk
    5:  1,   # Asphalt / Tar surface    <- merged with Concrete
    ...
    9:  4,   # Curb Up
   10:  4,   # Curb Down
   12:  5,   # Paving Blocks (Rough)
}

SUPER_NAMES = {
    0: "Paving Blocks / Rect. Tiles",
    1: "Concrete + Asphalt",
    2: "Brick (Smooth + Rough)",
    3: "Indoor (Carpet/Lino/Tile)",
    4: "Curb (Up + Down)",
    5: "Paving Blocks (Rough)",
}

N_SUPER_CLASSES = len(SUPER_NAMES)   # 7 — used to set K sweep range
...
def main():
    ...
    print("  Super-class distribution (train):")
    ...
    if has_unl and len(X_u) > 0:
        ...
        ts_u, um_u = project(unl_pca, "Unlabeled")

        pseudo_plot = pred_all.copy().astype(int)
        pseudo_plot[~keep_mask] = -1

        for coords, pname in [(ts_u, "t-SNE"), (um_u, "UMAP")]:
            ...
            plt.tight_layout()
            fname = f"22_unlabeled_{pname.lower()}.png"
            plt.savefig(fname, dpi=150, bbox_inches="tight"); plt.show()
            print(f"  Saved {fname}")

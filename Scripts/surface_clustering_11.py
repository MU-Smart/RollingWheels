...
CONFIG = {
    ...
    # Model
    "embedding_dim" : 128,          # wider bottleneck for 12 classes

    # Phase 1 — supervised contrastive
    "epochs_phase1"  : 300,         # more epochs — complex 12-class problem
    "batch_size"     : 512,         # larger batch = more negatives per step
    "lr"             : 3e-4,
    "temperature"    : 0.05,        # sharper distribution → tighter clusters
    "hard_neg_ratio" : 0.8,         # mine more hard negatives (was 0.5)

    # Margin loss: pushes similar surfaces apart even when already separated
    "margin"         : 0.4,
    "margin_weight"  : 0.3,         # blended with SupCon loss
...
class FeatureAttentionGate(nn.Module):
    """
    Input-level soft attention over the F feature dimensions.

    Each input sample gets its own attention weights — a vector of F scalars
    in (0,1) that reweight the raw features before the first linear layer.

    Why: handcrafted features have very different relevance (e.g. spectral
    centroid may be highly discriminative, raw variance less so).  Instead
    of letting the first Linear layer figure this out from scratch, an
    explicit gate learns a per-sample saliency map.

    Architecture:
        x  (B, F)
        → Linear(F, F//4) → GELU          # compress to find feature patterns
        → Linear(F//4, F) → Sigmoid        # gate in (0,1) per feature
        → gate * x                         # soft feature selection
    """
    def __init__(self, feature_dim: int):
        ...
        self.gate = nn.Sequential(
            ...
            nn.Linear(hidden, feature_dim), nn.Sigmoid(),
        )

    def forward(self, x):
        return self.gate(x) * x            # (B, F) — gated features


class CrossLayerAttention(nn.Module):
    """
    Single-head scaled dot-product attention between two hidden layers.

    Used between the 512-d and 256-d layers to let the deeper layer
    selectively attend back to the richer earlier representation.

    Query  = current layer output  h2  (B, d_low)
    Key    = skip layer output     h1  (B, d_high)   projected to d_low
    Value  = skip layer output     h1  (B, d_high)   projected to d_low

    The attended value is added to h2 (residual), then layer-normalised.

    Why: early layers see more raw feature detail; later layers are more
    abstract.  Cross-layer attention lets the model re-examine fine-grained
    patterns from layer 1 when forming the layer-2 representation, similar
    to how skip connections work in U-Net but with learned soft routing.

    Architecture:
        h1 (B, d_high) → Linear → k, v  (B, d_low)
        h2 (B, d_low)  → Linear → q     (B, d_low)
        attn = softmax(q·kᵀ / √d_low)    scalar per sample (B,)
        out  = h2 + attn * v
        out  = LayerNorm(out)
    """
    def __init__(self, d_high: int, d_low: int):
        super().__init__()
        self.scale  = d_low ** -0.5
        self.proj_k = nn.Linear(d_high, d_low, bias=False)
        self.proj_v = nn.Linear(d_high, d_low, bias=False)
        self.proj_q = nn.Linear(d_low,  d_low, bias=False)
        self.norm   = nn.LayerNorm(d_low)

    def forward(self, h1, h2):
        """h1: (B, d_high) skip,  h2: (B, d_low) current."""
        q   = self.proj_q(h2)                          # (B, d_low)
        k   = self.proj_k(h1)                          # (B, d_low)
        v   = self.proj_v(h1)                          # (B, d_low)
        # Scalar attention weight per sample
        attn = torch.sum(q * k, dim=-1, keepdim=True) * self.scale   # (B,1)
        attn = torch.sigmoid(attn)                     # soft gate in (0,1)
        out  = self.norm(h2 + attn * v)                # residual + LN
        return out
...
class MLPEncoder(nn.Module):
    """
    Attention-augmented MLP encoder.

    Forward pass:
        x  (B, F)
        │
        ├─ FeatureAttentionGate          learns which input features matter
        │       ↓
        ├─ Linear(F→512)+BN+GELU+Drop   layer 1  (h1)
        │       ↓
        ├─ Linear(512→256)+BN+GELU+Drop layer 2  (h2)
        │       ↓
        ├─ CrossLayerAttention(h1→h2)   h2 re-attends to h1 detail
        │       ↓
        ├─ Linear(256→128)+BN+GELU      layer 3  (h3)
        │       ↓
        └─ Linear(128→embed)            bottleneck z
                ↓ L2-normalise
               z̃  → used for clustering

        z̃ → projector (64→256→128) → p̃  → used for SupCon loss only
    """
    def __init__(self, feature_dim: int, embedding_dim: int = 64, proj_dim: int = 128):
        super().__init__()

        # Input-level feature attention gate
        self.feat_attn = FeatureAttentionGate(feature_dim)
        ...
        # Cross-layer attention: h2 attends back to h1
        self.cross_attn = CrossLayerAttention(d_high=512, d_low=256)
        ...
        # Projection head (training only — discarded at inference)
        self.projector = nn.Sequential(
            nn.Linear(embedding_dim, 256), nn.BatchNorm1d(256), nn.ReLU(),
            nn.Linear(256, proj_dim),
        )

        print(f"  MLPEncoder (attention-augmented):")
        print(f"    FeatureAttentionGate({feature_dim})")
        print(f"    Linear {feature_dim}→512 → CrossLayerAttention(512,256) → 256→128→{embedding_dim}")
        print(f"    Projector: {embedding_dim}→256→{proj_dim}")

    def embed(self, x):
        # Feature gate
        x  = self.feat_attn(x)          # (B, F) — gated input

        # Forward through layers with cross-layer attention
        h1 = self.layer1(x)             # (B, 512)
        h2 = self.layer2(h1)            # (B, 256)
        h2 = self.cross_attn(h1, h2)    # (B, 256) — h2 re-attends to h1
        h3 = self.layer3(h2)            # (B, 128)
        z  = self.fc_embed(h3)          # (B, embedding_dim)
    ...
    def get_feature_weights(self, x):
        """
        Return the attention gate weights for a batch of inputs.
        Useful for inspecting which features the model finds most important.
        Returns (B, F) tensor of gate values in (0,1).
        """
        with torch.no_grad():
            xt = torch.tensor(x, dtype=torch.float32) if isinstance(x, np.ndarray) else x
            return self.feat_attn.gate(xt).cpu().numpy()


# ---------------------------------------------------------------------------
# Margin loss  (pushes inter-class distance > margin, pulls intra < margin)
# ---------------------------------------------------------------------------
class MarginContrastiveLoss(nn.Module):
    """
    For every labeled pair (i,j) in the batch:
      - Same surface  → penalise if cosine distance > margin  (pull together)
      - Diff surface  → penalise if cosine distance < margin  (push apart)

    This complements SupCon which only operates on similarities.
    Margin loss explicitly enforces a minimum separation between classes,
    which is exactly what the overlapping outdoor surfaces need.
    """
    ...
    def forward(self, z, labels):
        ...
        sim  = z @ z.T                                     # (B,B) similarities
        dist = 1.0 - sim                                   # (B,B) distances
        eye  = torch.eye(z.size(0), dtype=torch.bool, device=z.device)
        lbl2d     = labels.unsqueeze(1)
        same_mask = (lbl2d == lbl2d.T) & ~eye
        diff_mask = (lbl2d != lbl2d.T) & ~eye

        # Pull: same-class pairs should be closer than margin
        pull = (same_mask * F.relu(dist - self.margin)).sum() / same_mask.sum().clamp(1)
        # Push: diff-class pairs should be farther than margin
        push = (diff_mask * F.relu(self.margin - dist)).sum() / diff_mask.sum().clamp(1)
        return pull + push
...
def train_supervised(model, X, labels, config, device, tag="Phase1"):
    ...
    supcon    = HardNegSupConLoss(config["temperature"], config["hard_neg_ratio"]).to(device)
    margin_fn = MarginContrastiveLoss(config.get("margin", 0.4)).to(device)
    mw        = config.get("margin_weight", 0.3)
    ...
    for epoch in range(epochs):
        ...
        for xb, yb in loader:
            ...
            optimizer.zero_grad()
            _, proj = model(xb)
            loss    = supcon(proj, yb) + mw * margin_fn(proj, yb)
            loss.backward()
...
def train_semisupervised(model, X_lab, y_lab, X_psd, y_psd, config, device):
    ...
    for epoch in range(epochs):
        ...
        for (xl, yl), (xp, yp) in zip(lab_loader, psd_loader):
            ...
            _, pl = model(xl)
            _, pp = model(xp)
            loss  = (criterion(pl, yl) + mw * margin_fn(pl, yl)
                     + pw * (criterion(pp, yp) + mw * margin_fn(pp, yp)))
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            total += loss.item()
...
def main():
    ...
    # Feature attention weights — which input features does the model attend to?
    print("  Feature attention weights (mean gate activation per feature)...")
    gate_weights = model.get_feature_weights(
        torch.tensor(tr_X, dtype=torch.float32).to(device)
    )                                                    # (N_train, F)
    mean_weights = gate_weights.mean(axis=0)             # (F,) mean over samples
    top_n        = min(30, len(feature_cols))
    top_idx      = np.argsort(mean_weights)[::-1][:top_n]

    fig, ax = plt.subplots(figsize=(12, 5))
    ax.bar(range(top_n),
           mean_weights[top_idx],
           color=["#e63946" if i < 5 else "#457b9d" for i in range(top_n)])
    ax.set_xticks(range(top_n))
    ax.set_xticklabels([feature_cols[i] for i in top_idx],
                       rotation=45, ha="right", fontsize=7)
    ax.set_ylabel("Mean attention gate weight")
    ax.set_title(f"Top-{top_n} most attended features (FeatureAttentionGate)")
    ax.grid(axis="y", alpha=0.3)
    plt.tight_layout()
    plt.savefig("feature_attention_weights.png", dpi=150, bbox_inches="tight")
    plt.show()
    print("  Saved feature_attention_weights.png")
    print("  Top-5 most attended features:")
    for rank, idx in enumerate(top_idx[:5], 1):
        print(f"    {rank}. {feature_cols[idx]:40s}  weight={mean_weights[idx]:.4f}")

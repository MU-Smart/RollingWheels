...
# ---------------------------------------------------------------------------
def estimate_sampling_hz(df: pd.DataFrame, fallback_hz: float = 50.0) -> float:
    """
    Estimate sampling frequency from the timestamp column.

    Handles three Android timestamp formats:
      - nanoseconds  (values ~ 1e18, diffs ~ 1e7..1e8)
      - milliseconds (values ~ 1e12, diffs ~ 1e1..1e3)
      - seconds      (values ~ 1e9,  diffs ~ 0.001..0.1)

    Returns fallback_hz if estimation fails or yields an implausible result.
    """
    time_cols = [c for c in df.columns if re.search(r"time|timestamp", c, re.I)]
    if time_cols:
        ...
        if len(ts) > 1:
            ...
            # Detect unit by order of magnitude of the median diff
            if median_dt >= 1e7:          # nanoseconds  (dt ~10ms = 1e7 ns)
                hz = 1e9 / median_dt
            elif median_dt >= 1.0:        # milliseconds (dt ~10ms = 10 ms)
                hz = 1e3 / median_dt
            else:                         # seconds      (dt ~0.01 s)
                hz = 1.0 / median_dt

            # Sanity check: accept only 1..2000 Hz
            if 1.0 <= hz <= 2000.0:
                return hz
...
def resample_signal(xyz: np.ndarray, src_hz: float, tgt_hz: float) -> np.ndarray:
    ...
    if abs(src_hz - tgt_hz) < 1.0:
        return xyz
    src_int = max(1, int(round(src_hz)))
    tgt_int = max(1, int(round(tgt_hz)))
    g    = gcd(tgt_int, src_int)
    up   = tgt_int // g
    down = src_int // g
    if up < 1 or down < 1:
        return xyz   # fallback: skip resampling rather than crash
...
def extract_windows(files_df, config):
    ...
    for fid, row in tqdm(files_df.iterrows(), total=len(files_df), desc="Windowing"):
        ...
        freq_log[fid] = src_hz
        if fid < 3:   # Print first few files for debugging
            print(f"    [{fid}] cols={list(df.columns)}  rows={len(df)}  detected={src_hz:.1f} Hz")
...
class ConvEncoder(nn.Module):
    def __init__(self, window_size, embedding_dim=64, proj_dim=128, in_channels=9):
        ...
        # Dynamically compute the flattened size to avoid hardcoded window_size // 8
        # (integer ceil division through stride-2 convs doesn't equal W//8 for all W)
        with torch.no_grad():
            _dummy = torch.zeros(1, in_channels, window_size)
            _flat  = int(self.flatten(self.encoder(_dummy)).shape[1])
        print(f"  ConvEncoder: W={window_size}  in_ch={in_channels}  flat={_flat}  embed={embedding_dim}")
        self.fc_embed = nn.Linear(_flat, embedding_dim)

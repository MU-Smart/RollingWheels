...
CONFIG = {
    ...
    "lr"             : 3e-4,
    "recon_w"        : 1.0,
    "contrast_w"     : 3.0,   # FIX 4: raised from 2.0
    "temperature"    : 0.07,
    "hard_neg_ratio" : 0.5,
    "warmup_epochs"  : 5,     # FIX 3: reduced from 15
...
class ConvAutoencoder(nn.Module):
    def __init__(self, window_size, embedding_dim=64, in_channels=3):
        super().__init__()
        ...
        self.decoder = nn.Sequential(
            ...
            nn.ConvTranspose1d(32,  in_channels, 5, stride=2, padding=2, output_padding=1),
        )
    ...
    def decode(self, z):
        ...
        return out.permute(0, 2, 1)       # (B,C,W) -> (B,W,C)
...
# ---------------------------------------------------------------------------
# 6. Training  (FIX 3: shorter warm-up | FIX 4: balanced sampler)
# ---------------------------------------------------------------------------
def train(model, windows, labels, config, device):
    X = torch.tensor(windows, dtype=torch.float32)
    Y = torch.tensor(labels,  dtype=torch.long)

    # FIX 4b: balanced sampler ensures equal surface contribution per batch
    ...
    for epoch in range(config["epochs"]):
        model.train()
        tot = rec_s = con_s = 0.0
        phase = "warm-up" if epoch < warmup else "joint  "
        ...
        for xb, yb in loader:
            ...
            if epoch < warmup:
                loss  = l_rec
                l_con = torch.tensor(0.0)
            else:
                l_con = criterion(z, yb)
                loss  = config["recon_w"] * l_rec + config["contrast_w"] * l_con

            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 1.0)
            optimizer.step()
            tot  += loss.item(); rec_s += l_rec.item(); con_s += l_con.item()

        scheduler.step()
        n = len(loader)
        print(f"Epoch {epoch+1:3d}/{config['epochs']} [{phase}] | "
              f"Total {tot/n:.4f} | Recon {rec_s/n:.4f} | Contrast {con_s/n:.4f}")
...
def get_embeddings(model, windows, device, batch_size=256):
    ...
    for i in range(0, len(X), batch_size):
        out.append(model.encode(X[i:i+batch_size].to(device)).cpu())
...
def main():
    ...
    # --- 4. Train (FIX 3+4: shorter warmup + balanced batches) ---

...
# ── Module D: Temporal Self-Attention Block ───────────────────────────────────
class TemporalAttentionBlock(nn.Module):
    """
    Multi-head self-attention across time steps.
    Each time step attends to all others, preserving the full-window vibration
    pattern that characterises each surface type.
    Input / output: (B, C, T)
    """
    def __init__(self, channels: int, num_heads: int = 8):
        super().__init__()
        nh = num_heads if channels % num_heads == 0 else 1
        self.attn = nn.MultiheadAttention(channels, nh, batch_first=True)
        self.norm = nn.LayerNorm(channels)

    def forward(self, x):
        xt = x.permute(0, 2, 1)              # (B, T, C) — time as sequence dim
        attn_out, _ = self.attn(xt, xt, xt)  # (B, T, C)
        xt = self.norm(attn_out + xt)         # residual + LayerNorm
...
class VibClustNet(nn.Module):
    ...
    def __init__(self, T: int, emb_dim: int = 256):
        ...
        # Full-window temporal self-attention (time steps attend to each other)
        self.temporal_attn = TemporalAttentionBlock(CH, num_heads=8)
        # Encoder head
    ...
    def _encode(self, x):
        ...
        after3           = self.temporal_attn(after3)           # (B, CH, T) — full-window attention

...
def project(pca_emb, tag):
    ...
    print(f"  t-SNE [{tag}]..."); ts = TSNE(2, random_state=42, perplexity=perp, max_iter=1000).fit_transform(pca_emb)
    print(f"  UMAP  [{tag}]...");  um = umap.UMAP(2, random_state=42, n_neighbors=min(15,len(pca_emb)-1), min_dist=0.1).fit_transform(pca_emb)
    return ts, um

def plot_grid(tsne, umap_, pred, c2s, gt, prefix):
    """2-row × 4-col grid. Rows = projection method. Cols = clustering + GT."""
    ...
    for row, (coords, pname) in enumerate([(tsne, "t-SNE"), (umap_, "UMAP")]):
        for col, (key, leg_fn, title) in enumerate(methods):
            _scatter(axes[row,col], coords, pred[key], leg_fn, f"{pname} — {title}", pname)
        # Ground truth panel
        ax = axes[row, 3]
        ...
        ax.set_title(f"{pname} — Ground Truth (true surface_id)", fontsize=9, fontweight="bold")
...
def main():
    ...
    plot_grid(ts, um, te_pred, te_c2s, te_y, "Test split — handcrafted features")

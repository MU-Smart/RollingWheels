...
CONFIG = {
    ...
    'surface_types_csv'     : Path('../Datasets/surface_types.csv'),  # columns: surface_id, surface_name
    'window_size'           : 1024,
    'overlap'               : 0.5,
    ...
    # Train / test split (split by FILE not by window)
    'test_size'      : 0.2,
    'split_seed'     : 42,
}
...
def extract_windows(files_df, config):
    ...
    acc_windows,  acc_labels,  acc_file_ids  = [], [], []
    gyro_windows, gyro_labels, gyro_file_ids = [], [], []

    for file_index, row in tqdm(files_df.iterrows(), total=len(files_df), desc="Windowing"):
        ...
        for start in range(0, len(data_df) - WINDOW_SIZE + 1, STEP_SIZE):
            ...
            if 'accelerometer' in file_path.lower():
                ...
                acc_file_ids.append(file_index)
            elif 'gyroscope' in file_path.lower():
                gyro_windows.append(xyz)
                gyro_labels.append(surface_id)
                gyro_file_ids.append(file_index)

    print(f"Accelerometer windows: {len(acc_windows)} | Gyroscope windows: {len(gyro_windows)}")
    return (np.array(acc_windows,  dtype=np.float32), np.array(acc_labels),  np.array(acc_file_ids),
            np.array(gyro_windows, dtype=np.float32), np.array(gyro_labels), np.array(gyro_file_ids))
...
# ──────────────────────────────────────────────────────────────────────────────
# 3b. File-based train / test split
# Splits at the FILE level so no recording leaks across train/test.
# Window-level random split leaks: adjacent windows from the same file are
# nearly identical — model memorises the file, not the surface type.
# ──────────────────────────────────────────────────────────────────────────────
from sklearn.model_selection import GroupShuffleSplit

def split_by_file(windows, labels, file_ids, test_size=0.2, seed=42):
    gss = GroupShuffleSplit(n_splits=1, test_size=test_size, random_state=seed)
    train_idx, test_idx = next(gss.split(windows, labels, groups=file_ids))
    n_tr = len(set(file_ids[train_idx].tolist()))
    n_te = len(set(file_ids[test_idx].tolist()))
    print(f"  Split: {len(train_idx)} train windows ({n_tr} files) | "
          f"{len(test_idx)} test windows ({n_te} files)")
...
# ─────────────────────────────────────────────────────────────────────────────
# Populated at runtime by load_surface_names() — do not edit manually.
SURFACE_NAMES: dict = {}

def load_surface_names(csv_path):
    """
    Load surface_id -> surface_name from CSV (columns: surface_id, surface_name).
    Populates the global SURFACE_NAMES dict used everywhere in this file.
    """
    global SURFACE_NAMES
    df = pd.read_csv(csv_path)
    # Accept common column-name variants (surface_id / id, surface_name / name)
    id_col   = next(c for c in df.columns if 'id'   in c.lower())
    name_col = next(c for c in df.columns if 'name' in c.lower())
    SURFACE_NAMES = {int(row[id_col]): str(row[name_col]) for _, row in df.iterrows()}
    print(f'  Loaded {len(SURFACE_NAMES)} surface types from {csv_path}:')
...
def print_comparison(emb_norm, emb_pca, pred_labels, gt_labels, split=''):
    rows = {
...
def plot_tsne_2d(tsne_coords, pred_labels, cluster_surface_maps, gt_labels,
                 title_prefix="Accelerometer", name='tsne_2d.png'):
...
def main():
    ...
    # ── Load surface type names ───────────────────────────────────────────────
    print("\n[0/7] Loading surface type names...")
    ...
    print("\n[2/7] Windowing...")
    (acc_windows,  acc_labels,  acc_file_ids,
     gyro_windows, gyro_labels, gyro_file_ids) = extract_windows(files_df, CONFIG)

    print("\n[3/7] Normalising & splitting by file...")
    acc_norm = normalise_windows(acc_windows)
    print(f"  Classes: {np.unique(acc_labels)}  ({len(np.unique(acc_labels))} total)")

    # Split BEFORE training -- model never sees test windows during training
    (train_windows, train_labels, train_file_ids,
     test_windows,  test_labels,  test_file_ids) = split_by_file(
        acc_norm, acc_labels, acc_file_ids,
        test_size=CONFIG['test_size'],
        seed=CONFIG['split_seed'],
    )

    print("\n[4/7] Training autoencoder (train split only)...")
    model = ConvAutoencoder(window_size=CONFIG['window_size'],
                            embedding_dim=CONFIG['embedding_dim']).to(device)
    model = train(model, train_windows, train_labels, CONFIG, device)

    print("\n[5/7] Extracting embeddings...")
    train_emb = get_embeddings(model, train_windows, device)
    test_emb  = get_embeddings(model, test_windows,  device)
    print(f"  Train: {train_emb.shape} | Test: {test_emb.shape}")

    # PCA: fit on TRAIN only, apply same transform to test (no leakage)
    print("  Post-processing (PCA fit on train, applied to both)...")
    train_emb_norm, train_emb_pca, pca_model = postprocess(
        train_emb, CONFIG['pca_variance_threshold']
    )
    test_emb_norm = normalize(test_emb, norm='l2', axis=1)
    test_emb_pca  = pca_model.transform(test_emb_norm)
    print(f"  Test PCA shape: {test_emb_pca.shape}")

    print("\n[6/7] Clustering...")
    n_unique = len(np.unique(train_labels))
    best_k, _ = auto_select_k(train_emb_pca,
                               k_min=max(2, n_unique - 2),
                               k_max=n_unique + 3)

    km  = KMeans(n_clusters=best_k, random_state=42, n_init=20).fit(train_emb_pca)
    agg = AgglomerativeClustering(n_clusters=best_k, metric='cosine', linkage='average')
    agg.fit(train_emb_norm)
    nn_m = NearestNeighbors(n_neighbors=10).fit(train_emb_pca)
    d, _ = nn_m.kneighbors(train_emb_pca)
    eps_auto = float(np.percentile(d[:, -1], 90))
    db  = DBSCAN(eps=eps_auto, min_samples=10).fit(train_emb_pca)

    train_pred = {'kmeans': km.labels_, 'agg': agg.labels_, 'dbscan': db.labels_}

    # Agglomerative has no .predict() -- KNN transfer from train assignments
    knn_agg = KNeighborsClassifier(n_neighbors=5).fit(train_emb_norm, agg.labels_)
    test_pred = {
        'kmeans': km.predict(test_emb_pca),
        'agg'   : knn_agg.predict(test_emb_norm),
        'dbscan': DBSCAN(eps=eps_auto, min_samples=10).fit_predict(test_emb_pca),
    }

    train_c2s = {m: map_clusters_to_surfaces(train_pred[m], train_labels) for m in train_pred}
    test_c2s  = {m: map_clusters_to_surfaces(test_pred[m],  test_labels)  for m in test_pred}

    print("\n  Cluster to Surface mapping (K-Means, train):")
    for cid, sid in sorted(train_c2s['kmeans'].items()):
        print(f"    Cluster {cid:2d} -> {surface_name(sid)} (id={sid})")

    train_df = print_comparison(train_emb_norm, train_emb_pca, train_pred,
                                train_labels, split='TRAIN')
    test_df  = print_comparison(test_emb_norm,  test_emb_pca,  test_pred,
                                test_labels,  split='TEST')

    print("\nGeneralisation Gap (Train ARI minus Test ARI)")
    print("  gap < 0.10=good | 0.10-0.20=acceptable | >0.20=overfit")
    for method in ['KMeans', 'Agglomerative', 'DBSCAN']:
        tr  = train_df.loc[method, 'ARI'] if not pd.isna(train_df.loc[method, 'ARI']) else 0.0
        te  = test_df.loc[method,  'ARI'] if not pd.isna(test_df.loc[method,  'ARI']) else 0.0
        gap = tr - te
        flag = 'good' if gap < 0.10 else 'overfit' if gap > 0.20 else 'acceptable'
        print(f"  {method:15s}  train={tr:.4f}  test={te:.4f}  gap={gap:+.4f}  [{flag}]")

    print("\n[7/7] Visualising...")
    for split_name, emb_pca, emb_norm, pred, c2s, gt in [
        ('Train', train_emb_pca, train_emb_norm, train_pred, train_c2s, train_labels),
        ('Test',  test_emb_pca,  test_emb_norm,  test_pred,  test_c2s,  test_labels),
    ]:
        print(f"  t-SNE [{split_name}] ...")
        perp = min(30, len(emb_pca) - 1)
        coords_2d = TSNE(n_components=2, random_state=42,
                         perplexity=perp, max_iter=1000).fit_transform(emb_pca)
        coords_3d = TSNE(n_components=3, random_state=42,
                         perplexity=perp, max_iter=1000).fit_transform(emb_pca)
        plot_tsne_2d(coords_2d, pred, c2s, gt,
                     title_prefix=f"Accelerometer [{split_name}]", name=f"tsne_2d_{split_name}.png")
        plot_tsne_3d(coords_3d, pred, c2s, gt,
                     title_prefix=f"Accelerometer [{split_name}]")

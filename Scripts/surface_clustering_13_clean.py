...
def experiment_fixed_k(pca_emb, norm_emb, gt, tsne_coords,
                       c2s_fn, ks=(3, 4, 5)):
    """
    Run KMeans for each k in ks, print metrics side-by-side,
    and plot t-SNE coloured by each k's cluster assignment.
    Useful for understanding how granularity affects cluster quality.
    """
    print(f"\n  Fixed-K experiment  (k = {list(ks)}):")
    rows = {}
    fig, axes = plt.subplots(1, len(ks), figsize=(7*len(ks), 6))
    fig.suptitle("Fixed-K experiment (t-SNE, test split)", fontweight="bold")
    ...
    plt.show(); print("  Saved fixed_k_experiment.png")
...
def main():
    ...
    experiment_fixed_k(te_pca, te_norm, te_y, ts, c2s_map, ks=(3, 4, 5))

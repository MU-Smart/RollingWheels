...
def setup_logging(log_dir: Path = Path(".")) -> logging.Logger:
    ...
    log_path = log_dir / f"run_{RUN_TS}_27_gpu.log"
...
CONFIG = {
    ...
    "output_csv"         : Path("27_unlabeled_predictions.csv"),
    ...
    "vcn_checkpoint"     : Path("vibclustnet_best_27.pth"),
    "vcn_cls_weight"     : 1.0,    # weight for classification auxiliary loss

    # Self-supervised / semi-supervised enhancements
    "vcn_freq_weight"       : 0.5,   # spectral reconstruction loss weight (0 = off)
    "vcn_contrast_weight"   : 0.3,   # NT-Xent contrastive loss weight (0 = off)
    "vcn_contrast_temp"     : 0.1,   # temperature for NT-Xent softmax
    "vcn_noise_std"         : 0.05,  # Gaussian noise std for contrastive augmentation
    "vcn_pseudo_weight"     : 0.5,   # pseudo-label loss weight for unlabeled (0 = off)
    "vcn_pseudo_threshold"  : 0.85,  # min softmax confidence to accept a pseudo-label
    "vcn_warmup_epochs"     : 20,    # epochs before pseudo-labeling starts
...
def multi_rec_loss(x_orig, recon, freq_weight: float = 0.0):
    """
    Log-cosh reconstruction loss in time domain + optional spectral domain.

    Time-domain log-cosh:
        Σᵢ log(cosh(yᵢ − ŷᵢ)) / n
        ≈ ½d² for small residuals (smooth like MSE),
          |d| for large ones (robust like MAE).

    Spectral component (freq_weight > 0):
        Same log-cosh applied to the rfft magnitude spectra.  Penalising
        frequency-magnitude errors directly encourages the decoder to capture
        vibration harmonics that are key for surface discrimination — making the
        encoder more discriminative even without labels.
    """
    if x_orig.shape[1] != 3:
        x_orig = x_orig.permute(0, 2, 1)
    ln2 = torch.tensor(2.0, device=x_orig.device).log()

    # Time-domain log-cosh
    d         = recon - x_orig
    time_loss = (torch.logaddexp(d, -d) - ln2).sum() / d.numel()

    if freq_weight <= 0.0:
        return time_loss

    # Spectral log-cosh on rfft magnitude
    orig_mag  = torch.fft.rfft(x_orig, dim=-1).abs()
    recon_mag = torch.fft.rfft(recon,  dim=-1).abs()
    df        = recon_mag - orig_mag
    freq_loss = (torch.logaddexp(df, -df) - ln2).sum() / df.numel()

    return time_loss + freq_weight * freq_loss
...
def nt_xent_loss(emb1: torch.Tensor, emb2: torch.Tensor,
    ...
    """
    NT-Xent (normalised temperature-scaled cross entropy) between two batches of
    embeddings produced from augmented views of the same windows.

    Each window i forms a positive pair (i, i+B).  All other 2B-2 windows in the
    combined batch are negatives.  Applying this to unlabeled data gives the
    encoder a self-supervised discriminative objective: different windows should
    map to different regions of the embedding sphere, while augmented copies of
    the same window should be close together.
    """
    e1 = F.normalize(emb1, dim=-1)
    e2 = F.normalize(emb2, dim=-1)
    B  = e1.shape[0]
    z  = torch.cat([e1, e2], dim=0)               # (2B, D)
    sim = torch.matmul(z, z.T) / temperature       # (2B, 2B)
    # Remove self-similarities from the softmax denominator
    mask = torch.eye(2 * B, dtype=torch.bool, device=emb1.device)
    sim  = sim.masked_fill(mask, -9e15)
    # Positive for row i is column i+B (and vice-versa)
    labels = torch.cat([
...
def _load_windows(csv_path, has_labels):
    """Load a windowed CSV and return (arr, labels) where arr is (N,3,T).
    If has_labels=False, labels is an all-(-1) array (unlabeled)."""
...
def load_windowed_data(cfg):
    """
    Loads labeled and unlabeled CSV files produced by FeatureExtractor notebook.
    Labeled CSV   : has surface_id column with known positive surface IDs.
    Unlabeled CSV : no surface_id column (assigned y=-1).

    Returns:
        tr_X  (N_tr, 3, T)  — labeled-train + unlabeled windows
        tr_y  (N_tr,)       — super-class labels; -1 for unlabeled windows
        te_X  (N_te, 3, T)  — labeled-test windows only
        te_y  (N_te,)       — super-class labels for test
        X_u   (N_u,  3, T)  — unlabeled windows (subset of tr_X for reference)
    """
    # ── Labeled data ──────────────────────────────────────────────────────────
    arr_l, labels = _load_windows(cfg["windowed_csv_labeled"], has_labels=True)

    # Keep only truly labeled rows (drop any surface_id == unlabeled_id)
...
def train_vibclustnet(model, train_loader, val_loader, cfg, device,
    ...
    freq_w     = cfg.get("vcn_freq_weight",       0.5)
    contr_w    = cfg.get("vcn_contrast_weight",   0.3)
    contr_temp = cfg.get("vcn_contrast_temp",     0.1)
    noise_std  = cfg.get("vcn_noise_std",         0.05)
    pseudo_w   = cfg.get("vcn_pseudo_weight",     0.5)
    pseudo_thr = cfg.get("vcn_pseudo_threshold",  0.85)
    warmup_ep  = cfg.get("vcn_warmup_epochs",     20)

    for ep in range(cfg["vcn_epochs"]):
        # ── Train ────────────────────────────────────────────────────────────
        model.train()
        tr_rec = tr_cls = tr_ctr = tr_psl = 0.0
        for xb, yb in train_loader:
            xb, yb = xb.to(device), yb.to(device)
            opt.zero_grad()
            recon, emb, _, _ = model(xb)
            rec_loss = multi_rec_loss(xb, recon, freq_weight=freq_w)

            # ── Contrastive self-supervised loss (labeled + unlabeled) ────────
            # Augment with Gaussian noise and push embeddings of both views together.
            # This provides discriminative signal even for unlabeled samples.
            contr_loss = torch.tensor(0.0, device=xb.device)
            if contr_w > 0:
                xb_aug = xb + torch.randn_like(xb) * noise_std
                _, emb_aug, _, _ = model(xb_aug)
                contr_loss = nt_xent_loss(emb, emb_aug, temperature=contr_temp)

            # ── Supervised classification loss (labeled samples only) ─────────
            ...
            # ── Pseudo-label loss for unlabeled samples (after warmup) ────────
            # The classifier's high-confidence predictions are used as soft targets.
            # This bridges labeled supervision into the unlabeled embedding space.
            psl_loss = torch.tensor(0.0, device=xb.device)
            if (model.classifier is not None and pseudo_w > 0
                    and ep >= warmup_ep):
                unlabeled = yb < 0
                if unlabeled.any():
                    with torch.no_grad():
                        pseudo_probs  = F.softmax(
                            model.classify(emb[unlabeled].detach()), dim=-1)
                        max_p, pseudo_lbl = pseudo_probs.max(dim=-1)
                        confident = max_p >= pseudo_thr
                    if confident.any():
                        logits_u = model.classify(emb[unlabeled][confident])
                        psl_loss = F.cross_entropy(logits_u, pseudo_lbl[confident])

            loss = rec_loss + cls_w * cls_loss + contr_w * contr_loss + pseudo_w * psl_loss
            loss.backward()
            nn.utils.clip_grad_norm_(model.parameters(), 5.0)
            opt.step()
            tr_rec += rec_loss.item()
            tr_cls += cls_loss.item()
            tr_ctr += contr_loss.item()
            tr_psl += psl_loss.item()
        tr_rec /= len(train_loader)
        tr_cls /= len(train_loader)
        tr_ctr /= len(train_loader)
        tr_psl /= len(train_loader)
        ...
        with torch.no_grad():
            for xb, yb in val_loader:
                xb, yb = xb.to(device), yb.to(device)
                recon, emb, _, _ = model(xb)
                va_rec += multi_rec_loss(xb, recon, freq_weight=freq_w).item()
                if model.classifier is not None and cls_w > 0:
                    labeled = yb >= 0
                    if labeled.any():
                        logits   = model.classify(emb[labeled])
...
def evaluate_unsupervised(emb, pred, pseudo_gt=None, metric="euclidean"):
    """
    Unsupervised metrics + optional ARI/NMI against a pseudo ground truth.
    pseudo_gt: reference cluster labels (e.g. KMeans) used as proxy for GT.
               ARI/NMI will be NaN if None.
    """
    pred = np.asarray(pred); ok = pred != -1
...
def print_metrics(tr_norm, tr_pca, tr_pred, tr_lbl,
    ...
    if xu_pred is not None and xu_norm is not None and xu_pca is not None:
        km_ref = xu_pred["kmeans"]   # KMeans used as pseudo-GT for ARI/NMI
        rows = {}
        for name, (key, metric, use_pca) in _METHOD_CFG.items():
            emb = xu_pca if use_pca else xu_norm
            # KMeans vs itself → ARI=1, NMI=1 (sanity check)
...
def main():
    ...
    # Labeled portion of training set (for NMI probe and evaluation)
    labeled_mask  = tr_y >= 0
    ...
    # Train on all data (labeled + unlabeled); validate on labeled test only
    ...
    # NMI probe uses labeled training subset only (y=-1 rows excluded)
    ...
    # ── Embed unlabeled and project with the fitted PCA ────────────────────────
    xu_norm = xu_pca = None
    if len(X_u) > 0:

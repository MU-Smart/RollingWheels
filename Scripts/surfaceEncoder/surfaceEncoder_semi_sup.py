"""
Road Surface Classification — SurfaceEncoder Clustering Pipeline
...
2. Train SurfaceEncoder autoencoder (multi-depth reconstruction loss)
...
7. SurfaceEncoder diagnostic plots (attention, CAIM, reconstruction)
"""
...
CONFIG = {
    ...
    # SurfaceEncoder hyper-parameters
    "vcn_epochs"         : 150,
    "vcn_batch_size"     : 32,
    "vcn_lr"             : 1e-3,
    "vcn_patience"       : 50,
    "vcn_embedding_dim"  : 128,
    "vcn_checkpoint"     : Path("SurfaceEncoder_best_24.pth"),
    "vcn_cls_weight"     : 0,    # weight for classification auxiliary loss
...
# ══════════════════════════════════════════════════════════════════════════════
# SurfaceEncoder — raw-window autoencoder path
# ══════════════════════════════════════════════════════════════════════════════
...
# ── SurfaceEncoder training ───────────────────────────────────────────────────────
def train_SurfaceEncoder(model, train_loader, val_loader, cfg, device,
                      X_labeled=None, y_labeled=None):
    """
    Trains SurfaceEncoder autoencoder with multi_rec_loss.
...
def plot_SurfaceEncoder_diagnostics(model, te_X, te_pred_kmeans, device, figures_dir: Path):
    """
...
def main():
    ...
    logger.info("\n[3] Train SurfaceEncoder autoencoder")
    ...
    model = SurfaceEncoder(T=T, emb_dim=CONFIG["vcn_embedding_dim"],
                        n_classes=n_cls_for_clf).to(device)
    logger.info(f"  Parameters: {sum(p.numel() for p in model.parameters()):,}")
    # NMI probe uses labeled training subset only (y=-1 rows excluded)
    model = train_SurfaceEncoder(model, tr_loader, va_loader, CONFIG, device,
    ...
    logger.info("\n[10] SurfaceEncoder diagnostic plots")
    plot_SurfaceEncoder_diagnostics(model, te_X, te_pred["kmeans"], device, figures_dir)
